...
Demo 的儲存比照 UAT：保留原本掛在 Docker 預設路徑的 80G logical volume，改掛到 `/srv/platform`，原有 Docker 內容移到 `/srv/platform/docker`，並將 Docker data-root 指向該處。`/home/mobagel` 底下的專案 checkout 與應用持久資料搬到 `/srv/platform` 下對應位置；家目錄本身保留原處不動。UAT 從未把家目錄搬進 `/srv`，「結構參考 UAT」指的是應用資料的擺放，不是家目錄的搬遷。

與 UAT 的一個關鍵差異：UAT 當時是 0 containers、0 images 的空環境，Demo 不是。既有 tutorial 已明文警告非空環境不得直接照搬 remount 步驟，因此 Demo 的 Docker 資料搬移必須先確認容量、停妥服務並具備回復點，而不是重複 UAT 的指令序列。
...
Edge 新增一個 entrance port：`8082` DNAT 到 `172.23.57.12:443`，並依既有慣例同時建立對應的 forward 放行與來源 NAT，使 Demo 只信任 Edge 的私有側位址。`8082` 原本在既有 spec 中是臨時驗收 port，本功能將其語意改為常駐 entrance port，理由與代價記錄於 ADR-0001。Demo 舊有的對外直連（實體 bridge 上的 `10.1.2.57` 直接提供 80/443）整組移除；`443` 上具體由什麼服務提供，在 guest 盤點完成後決定。
...
- Demo's storage follows UAT's shape: the existing large logical volume currently mounted at Docker's default path is remounted at `/srv/platform`, its existing Docker content is relocated beneath that mount, and Docker's data-root is pointed there. The small existing `/srv` logical volume remains, with `/srv/platform` mounted below it.
- Demo's Docker state is not empty, unlike UAT's at the time its layout was built. The existing tutorial explicitly warns against copying the remount procedure into a non-empty environment. Demo therefore requires its own measured plan, service stop ordering and rollback, decided in Phase 2 from Phase 1's measurements.
...
- Whether the migration target has adequate capacity, and whether the large existing data volume is used instead, are Phase 2 decisions gated on Phase 1's measurements. If projected free space after migration falls below twenty percent, the migration stops as a `[HUMAN ACTION]` rather than proceeding or deleting data to make room.
...
- Demo's previous direct external listeners on the physical bridge are removed. What serves port `443` afterwards is decided after Phase 1's inventory; the requirement is that Demo is reached only through the entrance port.
...
- The secondary seam is also an existing one: **in-guest verification of storage placement and data integrity**, as used for UAT's storage acceptance. Through it: Docker's data-root, the project checkout and persistent application data must resolve below `/srv/platform`; existing Docker content must be present after the remount; and at least twenty percent free space must remain.
